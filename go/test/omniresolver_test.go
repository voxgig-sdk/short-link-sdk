...
import (
	...
	omni "github.com/voxgig-sdk/short-link-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ShortLinkError); is {
				out := map[string]any{
					"name":    "ShortLinkError",
...
func omniCtx(arg any, client *sdk.ShortLinkSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ShortLinkSDK); is {
			client = live
