...
//
// NewShortLinkSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.ShortLinkSDK {
	res, _ := kase["res"].([]any)
	...
	return sdk.NewShortLinkSDK(opts)
}
...
func fcCandidates(client *sdk.ShortLinkSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.ShortLinkSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.ShortLinkSDK, name string) any {
