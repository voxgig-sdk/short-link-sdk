...
module ShortLinkUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ShortLinkResponse.new({})
    ...
    if fetch_err
      ...
      response = ShortLinkResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ShortLinkResponse.new(fetched)
    else
