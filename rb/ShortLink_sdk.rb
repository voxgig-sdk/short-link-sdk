...
require_relative 'ShortLink_types'
...
class ShortLinkSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ShortLinkHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ShortLinkHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ShortLinkFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ShortLinkUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ShortLinkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ShortLinkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ShortLinkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ShortLinkHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ShortLinkError.new(
        "graphql_error", "ShortLinkSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
