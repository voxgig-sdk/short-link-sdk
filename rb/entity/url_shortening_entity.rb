...
class UrlShorteningEntity
  ...
  def make
    opts = @_entopts.dup
    UrlShorteningEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ShortLinkHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = ShortLinkHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  #   as an empty match, so client.UrlShortening.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [UrlShortening, Hash] the loaded UrlShortening; raises ShortLinkError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = ShortLinkHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
