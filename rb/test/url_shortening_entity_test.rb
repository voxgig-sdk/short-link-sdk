...
class UrlShorteningEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set SHORTLINK_TEST_URL_SHORTENING_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    url_shortening_ref01_data = nil
    if url_shortening_ref01_data_raw.length > 0
...
def url_shortening_basic_setup(extra)
  ...
  entid_env_raw = ENV["SHORTLINK_TEST_URL_SHORTENING_ENTID"]
  ...
  if env["SHORTLINK_TEST_LIVE"] == "TRUE"
    ...
    client = ShortLinkSDK.new(Helpers.to_map(merged_opts))
  end
