...
[MCP](https://modelcontextprotocol.io) server exposing the ShortLink SDK as
two agent tools — `short-link_list` and `short-link_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/short-link-mcp)
make build

# 2. Provide credentials via the environment
export SHORT_LINK_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user short-link \
  -- /absolute/path/to/short-link-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./short-link-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// short-link_list: first page of records
{ "entity": "url_shortening" }
{ "entity": "url_shortening", "query": { } }

// short-link_load: one record by id
{ "entity": "url_shortening", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/short-link-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user short-link \
     -- "$PWD"/dist/*/short-link-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `short-link_list` and `short-link_load` tools now appear
   in new sessions. Ask the agent to *"list url_shortening using short-link"*
   and it calls `short-link_list` with `{"entity":"url_shortening"}`.
...
```sh
export SHORT_LINK_APIKEY=sk_live_xxx            # API key
export SHORT_LINK_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `short-link_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `short-link_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `SHORT_LINK_APIKEY` | API key sent with every request. |
| `SHORT_LINK_BASE` | Optional override of the API base URL. |
...
```sh
./short-link-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"short-link_load","arguments":{"entity":"url_shortening","query":{"id":1}}}}'
```
