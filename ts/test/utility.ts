...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.SHORTLINK_TEST_LIVE ||
    'TRUE' === process.env.SHORTLINK_TEST_OVERRIDE
  ) {
  ...
  m.SHORTLINK_TEST_EXPLAIN = process.env.SHORTLINK_TEST_EXPLAIN || m.SHORTLINK_TEST_EXPLAIN
