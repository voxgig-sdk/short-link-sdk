...
const TEST_SEED = { "entity": { "url_shortening": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+ShortLinkSDK\b/.test(code) ||
        /\bShortLinkSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ShortLinkSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ShortLinkSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
