// Command ShortLink-cli is a CLI + REPL for the ShortLink SDK,
...
const prompt = "short-link"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "url_shortening"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      SHORT_LINK_APIKEY (api key), SHORT_LINK_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: SHORT_LINK_APIKEY carries the API key and
	// SHORT_LINK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("SHORT_LINK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("SHORT_LINK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewShortLinkSDK(opts)
