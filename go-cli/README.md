...
boru-driven command-line client **and** interactive REPL for the ShortLink
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/short-link-cli)
make build
...
./short-link-cli load 1 url_shortening            # {id:1} shorthand
./short-link-cli load '{id:1}' url_shortening       # explicit match map

# 5. Override the API base URL for a single call
SHORT_LINK_BASE=https://api.example.com ./short-link-cli load 1 url_shortening

# 6. No arguments -> interactive REPL
./short-link-cli
short-link> load 1 url_shortening
short-link> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/short-link-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/short-link-cli load 1 url_shortening
   ```
...
```sh
./short-link-cli load 1 url_shortening          # scalar shorthand for {id:1}
./short-link-cli load '{id:1}' url_shortening     # explicit match map
```
...
./short-link-cli load 1 url_shortening
```
...
Run with no arguments to open a REPL (prompt `short-link>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `url_shortening`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/short-link-cli`. |
...
(`./short-link-cli <expr>`) and interactively (the REPL), and expressions compose the
