...
// ShortLink SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class ShortLinkMakeError
{
    public static function call(?ShortLinkContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ShortLinkContext([], null);
        }
        $op = $ctx->op ?? new ShortLinkOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ShortLinkResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ShortLinkError) ? $err->msg : (string)$err;
        $msg = "ShortLinkSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new ShortLinkError('', $msg, $ctx);
