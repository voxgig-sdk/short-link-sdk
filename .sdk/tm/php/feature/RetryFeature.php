...
class ShortLinkRetryFeature extends ShortLinkBaseFeature
{
    ...
    public function init(ShortLinkContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (ShortLinkContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(ShortLinkContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(ShortLinkContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
