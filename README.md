...
Short Link API client, generated from the OpenAPI spec.

This API provides a simple interface for shortening URLs using GET requests.

Learn more about Short Link API at [github.com/marwan-pr/short_link](https://github.com/marwan-pr/short_link).

This is an unofficial SDK for the Short Link public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
```bash
cd go-mcp && go build -o short-link-mcp .
```
...
```json
{
  "mcpServers": {
    "short-link": {
      "command": "/abs/path/to/short-link-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **UrlShortening** | The UrlShortening entity (load). | `/api/set/index.php` |
...
// Load a specific urlshortening (returns the ENTITY; call data_get() for the record; throws on error)
$urlshortening = $client->UrlShortening()->load(["url" => "example_url"]);
...
```go
import sdk "github.com/voxgig-sdk/short-link-sdk/go"
...
# Load a specific urlshortening (returns the ENTITY; call data_get for the record)
urlshortening = client.UrlShortening.load({ "url" => "example_url" })
puts urlshortening
```
...
Generated from the Short Link API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
