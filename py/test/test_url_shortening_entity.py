    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set SHORT_LINK_TEST_URL_SHORTENING_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        url_shortening_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.url_shortening")))
        url_shortening_ref01_data = None
        if len(url_shortening_ref01_data_raw) > 0:
...
def _url_shortening_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("SHORT_LINK_TEST_URL_SHORTENING_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("SHORT_LINK_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = ShortLinkSDK(helpers.to_map(merged_opts))

    _live = env.get("SHORT_LINK_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("SHORT_LINK_TEST_EXPLAIN") == "TRUE",
        "live": _live,
