...
class ShortLinkTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ShortLinkTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ShortLinkTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ShortLinkTestRunner.getenv("SHORT_LINK_TEST_LIVE")
        override = ShortLinkTestRunner.getenv("SHORT_LINK_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ShortLinkTestRunner.getenv(key)
        ...
        explain = ShortLinkTestRunner.getenv("SHORT_LINK_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if ShortLinkTestRunner._test_control is not None:
            return ShortLinkTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ShortLinkTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ShortLinkTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ShortLinkTestRunner.load_test_control()
...
def load_env_local():
    ShortLinkTestRunner.load_env_local()


def env_override(m):
    return ShortLinkTestRunner.env_override(m)


def entity_data(v):
    return ShortLinkTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ShortLinkTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ShortLinkTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ShortLinkTestRunner.load_test_control()


def live_client_options():
    return ShortLinkTestRunner.live_client_options()


def live_delay_ms():
    return ShortLinkTestRunner.live_delay_ms()
