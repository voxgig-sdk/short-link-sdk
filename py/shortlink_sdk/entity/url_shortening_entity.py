...
from shortlink_sdk.shortlink_types import (
    UrlShortening,
...
class UrlShorteningEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return UrlShorteningEntity(self._client, opts)
    ...
    def data_get(self) -> UrlShortening:
    ...
    def match_get(self) -> UrlShortening:
    ...
    def load(self, reqmatch=None, ctrl=None) -> UrlShortening:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.UrlShortening().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
