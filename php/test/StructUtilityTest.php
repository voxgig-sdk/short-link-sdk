...
// ShortLink SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = ShortLinkOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(ShortLinkOmni::structfix($now, Struct::undef()))
                        !== json_encode(ShortLinkOmni::structfix($val, Struct::undef()))) {
