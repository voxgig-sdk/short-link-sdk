...
class FtCtrl extends ShortLinkControl
{
...
class FtClient
{
    ...
    private ShortLinkUtility $utility;

    public function __construct(ShortLinkUtility $utility, array $options)
    {
    ...
    public function get_utility(): ShortLinkUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ShortLinkContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ShortLinkContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ShortLinkContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ShortLinkConfig::make_feature($name);
    ...
    private static function buildUrl(ShortLinkSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ShortLinkError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ShortLinkResponse($fetched);
        }
    ...
    private function fail(ShortLinkContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ShortLinkContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
