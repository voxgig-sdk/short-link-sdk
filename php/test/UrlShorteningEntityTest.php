...
class UrlShorteningEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set SHORTLINK_TEST_URL_SHORTENING_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $url_shortening_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.url_shortening")));
        $url_shortening_ref01_data = null;
        if (count($url_shortening_ref01_data_raw) > 0) {
            $url_shortening_ref01_data = Helpers::to_map($url_shortening_ref01_data_raw[0][1]);
        }
...
function url_shortening_basic_setup($extra)
{
    ...
    $client = ShortLinkSDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("SHORTLINK_TEST_URL_SHORTENING_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["SHORTLINK_TEST_URL_SHORTENING_ENTID"]);
    ...
    if ($env["SHORTLINK_TEST_LIVE"] === "TRUE") {
        ...
        $client = new ShortLinkSDK(Helpers::to_map($merged_opts));
    }
