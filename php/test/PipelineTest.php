...
{
    private static function utility(): ShortLinkUtility
    {
        return new ShortLinkUtility();
    }
    ...
    private static function ctx(array $over = []): ShortLinkContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new ShortLinkContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new ShortLinkOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof ShortLinkError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new ShortLinkOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = ShortLinkMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new ShortLinkOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = ShortLinkMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new ShortLinkOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = ShortLinkMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ShortLinkSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = ShortLinkMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new ShortLinkResponse([]),
            'result' => new ShortLinkResult([]),
        ]);
        [, $err] = ShortLinkMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new ShortLinkSpec([]),
            'response' => null,
            'result' => new ShortLinkResult([]),
        ]);
        [, $err] = ShortLinkMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new ShortLinkSpec(['step' => 's']),
            'response' => new ShortLinkResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new ShortLinkResult([]),
        ]);
        [, $err] = ShortLinkMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new ShortLinkSpec(['step' => 's']),
            'response' => new ShortLinkResponse(self::resp(200, ['v' => 1])),
            'result' => new ShortLinkResult([]),
        ]);
        [, $err] = ShortLinkMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new ShortLinkSpec(['step' => 's']),
            'response' => new ShortLinkResponse(self::resp(200, ['v' => 2])),
            'result' => new ShortLinkResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        ShortLinkMakeResponse::call($ctx);
    ...
    {
        $preset = new ShortLinkResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new ShortLinkSpec([]),
            'response' => new ShortLinkResponse([]),
            'result' => new ShortLinkResult([]),
        ...
        [$got, $err] = ShortLinkMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new ShortLinkResult([])]);
        [, $err] = ShortLinkMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new ShortLinkSpec([]), 'result' => null]);
        [, $err] = ShortLinkMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ShortLinkSpec(['step' => 's']),
            'result' => new ShortLinkResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new ShortLinkOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ShortLinkMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new ShortLinkSpec(['step' => 's']),
            'result' => new ShortLinkResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new ShortLinkOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = ShortLinkMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new ShortLinkResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new ShortLinkSpec([]),
            'result' => new ShortLinkResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = ShortLinkMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ShortLinkMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new ShortLinkError('boom', 'boom');
        $utility->fetcher = function (ShortLinkContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ShortLinkSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = ShortLinkMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (ShortLinkContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = ShortLinkMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(ShortLinkResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (ShortLinkContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new ShortLinkSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = ShortLinkMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new ShortLinkResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new ShortLinkSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = ShortLinkMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = ShortLinkMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = ShortLinkMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new ShortLinkResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, ShortLinkDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new ShortLinkResult(['ok' => false])]);
        $this->expectException(ShortLinkError::class);
        ShortLinkDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new ShortLinkResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', ShortLinkMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        ShortLinkMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new ShortLinkBaseFeature();
        $b = new ShortLinkBaseFeature();
        ShortLinkFeatureAdd::call($ctx, $a);
        ShortLinkFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): ShortLinkBaseFeature
    {
        $f = new ShortLinkBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        ShortLinkFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): ShortLinkContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new ShortLinkSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = ShortLinkPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new ShortLinkResponse(['headers' => null]),
            'result' => new ShortLinkResult([]),
        ]);
        ShortLinkResultHeaders::call($ctx);
