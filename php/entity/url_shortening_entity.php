...
// ShortLink SDK UrlShortening entity
...
class UrlShorteningEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new UrlShorteningEntity($this->_client, $opts);
    }

    /**
     * @param UrlShortening|array $args UrlShortening data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ShortLinkHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return UrlShortening|array The current UrlShortening data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of UrlShortening fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ShortLinkHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of UrlShortening fields).
     */
     ...
     *
     * @param UrlShorteningLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed UrlShorteningLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return UrlShortening|array The loaded UrlShortening as an assoc-array at the
     *   SDK boundary; throws ShortLinkError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = ShortLinkHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
