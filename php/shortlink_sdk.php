...
class ShortLinkSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ShortLinkHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ShortLinkHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ShortLinkFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ShortLinkFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ShortLinkUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ShortLinkHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ShortLinkHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ShortLinkHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ShortLinkError($op . "_allow",
                "ShortLinkSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ShortLinkHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ShortLinkError("graphql_error",
                "ShortLinkSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function UrlShortening($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_url_shortening === null) {
                $this->_url_shortening = new UrlShorteningEntity($this, null);
            }
            return $this->_url_shortening;
        }
        return new UrlShorteningEntity($this, $data);
    }
