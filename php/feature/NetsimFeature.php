...
class ShortLinkNetsimFeature extends ShortLinkBaseFeature
{
    ...
    private function _simulate(ShortLinkContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(ShortLinkContext $ctx, array $applied): void
    {
        $client = $this->client;
